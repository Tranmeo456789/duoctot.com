...
<span class="mt-2 d-flex justify-content-center w-100 align-items-center">
    <span class="view-btn-add-product p-2 btn" data-type="{{$type??''}}" data-object="{{$typeObject??''}}" data-orderby="{{$typeOrderBy??'hang_moi'}}" data-idcat="{{$idCat??null}}" data-offset="{{$dataOffset??20}}" data-href="{{route('fe.product.loadMoreProducts')}}">
        <span size="20" class="mr-2 css-wi4pw5">
        ...
        <span class="css-15sc8tc">Xem thêm <span class="visibility-number-product">{{$countProduct??null}}</span> sản phẩm</span>
    </span>
</span>
@endif