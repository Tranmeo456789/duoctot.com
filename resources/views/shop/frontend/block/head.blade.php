...
<!-- <link href="{{ asset('/shop/template/css/bootstrap.min.css')}}?t=@php echo time() @endphp" rel="stylesheet" type="text/css"/>
...
<script>
    // Lấy tất cả các thẻ tiêu đề (h1, h2, h3, h4)
    var headers = document.querySelectorAll('h1, h2, h3, h4');

    // Tạo một phần tử <ul> để chứa danh sách mục lục
    var list = document.createElement('ul');
    list.id = 'toc'; // Đặt ID cho danh sách mục lục (tùy chọn)

    // Duyệt qua các thẻ tiêu đề và tạo các mục danh sách
    headers.forEach(function(header) {
        var listItem = document.createElement('li');
        var link = document.createElement('a');
        link.href = '#' + header.id; // Tạo liên kết đến thẻ tiêu đề
        link.textContent = header.textContent;
        listItem.appendChild(link);
        list.appendChild(listItem);
    });

    // Thêm danh sách mục lục vào body (hoặc vị trí mong muốn)
    document.body.appendChild(list);
</script>
<script>
    // Lấy tất cả các thẻ style trên trang
    var styles = document.getElementsByTagName("style");

    // Duyệt qua các thẻ style và sửa lỗi
    for (var i = 0; i < styles.length; i++) {
        ...
        try {
            ...
            sheet.cssRules; // Lấy các quy tắc CSS
        } catch (e) {
...
<script async src="https://cdn.ampproject.org/v0/amp-consent-0.1.mjs" custom-element="amp-consent" type="module" crossorigin="anonymous"></script>
...
<script async src="https://cdn.ampproject.org/v0/amp-geo-0.1.mjs" custom-element="amp-geo" type="module" crossorigin="anonymous"></script>
<script async nomodule src="https://cdn.ampproject.org/v0/amp-geo-0.1.js" crossorigin="anonymous" custom-element="amp-geo"></script>
